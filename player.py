...
class Player(Entity):
    def __init__(self, position: Vec3, speed = 3, jump_height = 14):
        super().__init__(
            model = "cube", 
            position = position,
            scale = (1.3, 1, 1.3), 
        ...
        # Specify raycast target for better performance
    ...
    def update(self):
        ...
        y_ray = raycast(origin = self.world_position, direction = (0, y_dir(self.velocity_y), 0), traverse_target = self.traverse_target if self.traverse_target is not None else scene, ignore = [self, ])
        ...
        if held_keys["w"]:
            ...
            self.velocity_z = lerp(self.velocity_z, 0 if y_ray.distance < 4 else 1, time.dt * 3)
        if held_keys["a"]:
        ...
        if self.movementX != 0:
            ...
            x_ray = raycast(origin = self.world_position, direction = direction, traverse_target = self.traverse_target if self.traverse_target is not None else scene, ignore = [self, ])
        ...
        if self.movementZ != 0:
            ...
            z_ray = raycast(origin = self.world_position, direction = direction, traverse_target = self.traverse_target if self.traverse_target is not None else scene, ignore = [self, ])
...
if __name__ == "__main__":
    ...
    ground = Entity(model = "cube", collider = "box", texture = "white_cube", scale = (100, 1, 100))
    cube = Entity(model = "cube", collider = "box", texture = "white_cube", scale = 5, position = (0, 2, 5))
